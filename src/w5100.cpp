...
/*!
 *  @brief  Increase the judgment of W5100s chip.
 */
uint8_t W5100Class::init(void)
{
	...
	initSS();
	resetSS();
	_SPIx->beginTransaction(_spi_settings);
...
/*!
 *  @brief  The lowest bit of MR in W5100S is not allowed to be written, it is always 1, and the default MR is 0x03 after reset.
 */
...
/*!
 *  @brief  Determine whether the current network chip is W5100s. 
 *			Because the MR definitions of W5100s and W5100 are different, isW5100() cannot determine the chip.
 *			The lowest bit of MR in W5100S is not allowed to be written, it is always 1, so writeMR is inconsistent with read.
 *			Added chip=50 TYPE.
 */
...
/*!
 *  @brief  Added link status support for W5100s.
 */
...
uint16_t W5100Class::write(uint16_t addr, const uint8_t *buf, uint16_t len)
{
	...
	if (chip == 50 || chip == 51) {
		for (uint16_t i=0; i<len; i++) {
			setSS();
			_SPIx->transfer(0xF0);
			_SPIx->transfer(addr >> 8);
			_SPIx->transfer(addr & 0xFF);
			addr++;
		...
		cmd[3] = len & 0xFF;
		_SPIx->transfer(cmd, 4);
#ifdef SPI_HAS_TRANSFER_BUF
		_SPIx->transfer(buf, NULL, len);
#else
		...
		if (len <= 5) {
			...
			_SPIx->transfer(cmd, len + 3);
		} else {
			_SPIx->transfer(cmd, 3);
#ifdef SPI_HAS_TRANSFER_BUF
			_SPIx->transfer(buf, NULL, len);
#else
			// TODO: copy 8 bytes at a time to cmd[] and block transfer
...
uint16_t W5100Class::read(uint16_t addr, uint8_t *buf, uint16_t len)
{
	...
	if (chip == 50 || chip == 51) {
		for (uint16_t i=0; i < len; i++) {
			...
			_SPIx->transfer(addr & 0xFF);
			addr++;
			buf[i] = _SPIx->transfer(0);
			#else
			cmd[0] = 0x0F;
			cmd[1] = addr >> 8;
			cmd[2] = addr & 0xFF;
			cmd[3] = 0;
			_SPIx->transfer(cmd, 4); // TODO: why doesn't this work?
			buf[i] = cmd[3];
			addr++;
			#endif
			resetSS();
